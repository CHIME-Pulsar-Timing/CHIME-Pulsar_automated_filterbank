...
def run_rfifind(fname, ext, dead_gpus=""):
    ...
    if ignore_chan_string == "":
        ...
        rfifind_command = (
            f"rfifind -blocks {pipeline_config.rfiblocks} -o {fname} {fname}{ext}"
        )
    else:
...
def run_kc_iqrm(fname, ext, ignorechans):
    ...
    if ignorechans == "":
        command = f"python {path}/rfi_pipeline.py --option 0,1,2,3,4,5,6,8 --outfilename {fname}{new_rfimask_name} --rfac 16 --include_rfifind {rfimask_fn}"
    else:
...
def dedisperse_data(fname, ext, rfi_ext, dm):
    # dedisperse the data
    #
    try:
        command = f"prepsubband -lodm {dm} -dmstep 0.1 -numdms 1 -nsub 1024 -downsamp 1 -mask {fname}{rfi_ext} -o {fname}_dedisp {fname}{ext}"
        run_dedisperse = subprocess.check_call([command],shell=True)
    except:
...
def run_sp(fname):
    ...
    failed = True
    try:
        run_sp_cmd = subprocess.check_call(
            [sp_command], shell=True, executable="/bin/bash"
        )
        # run_sp_cmd.wait()
    except subprocess.CalledProcessError:
        [logging.info(f) for f in os.listdir(".")]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sk_mask",
        action="store_true",
        help="apply a SK mask from 'your' onto the rfifind mask",
    )
    parser.add_argument(
        "--kc_iqrm", action="store_true", help="apply a Kathryn's IQRM mask"
    )
    parser.add_argument("--fil", type=str, help="Input filterbank file")
    parser.add_argument(
        "--dedisp", action="store_true", help="Run prepsubband and dedisperse the data"
    )
    parser.add_argument(
        "--dm",
        type=float,
    ...
    parser.add_argument(
        "--rfifind",
        action="store_true",
        help="Runs rfifind using the configuration in pipeline config",
    )
    parser.add_argument(
        "--dead_gpu",
        type=str,
        default="",
        help="use this option if you want to input a mask for dead GPUs",
    )
    parser.add_argument(
        "--slurm",
        type=str,
        help="specifies the root folder to output to, this can be useful on computecanada to reduce IO of files, we use the ${SLURM_TMPDIR} on CC",
    )
    parser.add_argument(
        "--log",
        type=str,
        help="name of file to write log to",
        default="automated_filterbank_batch_",
    )
    parser.add_argument(
        "--one_dm",
        action="store_true",
        help="only run the pipeline for one DM, this is useful for testing the pipeline",
    )
    ...
    if dedisp:
        if one_dm:
            dedisperse_data(fname, ext, mask_name, source_dm)
        else:
            # run ddplan
